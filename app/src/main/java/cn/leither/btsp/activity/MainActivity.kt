...
import cn.leither.btsp.command.GetWifiConnection
...
import cn.leither.btsp.state.MainState
...
class MainActivity : AppCompatActivity() {
    ...
    private fun createEnPasswordDialog(ssid: String, iface: String ){
        val input = state.sockets[0].inputStream
        val output = state.sockets[0].outputStream
        //val dialog = CreateConnDialogFragment(state, ssid, iface, input, output)
